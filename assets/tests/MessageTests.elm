...
import KbdEvent
import Message exposing (Context(..), Message(..))
...
suite =
    describe "Message"
        ...
        , describe "interpret" interpretTests
        ]

contextualizeTests : List Test
contextualizeTests =
    [ todo "ReceivePortMessage"
    , todo "KeyDown with enter key on any route"
    , todo "KeyUp with play chord key"
    , todo "KeyUp with a play note key"
    , todo "KeyDown with next item on SelectChord route"
    ]
    ...
    [ test "omits ReceivePortMessage on any route when sent by self" <|
        \_ ->
            let
                ...
                result =
                    Message.interpret msg (WithClientId "bach")
            in
            Expect.equal [] result
    , test "passes thru ReceivePortMessage on any route when sent by other" <|
        \_ ->
            let
                ...
                result =
                    Message.interpret msg (WithClientId "waititi")
            in
    ...
    , test "KeyDown with enter key on any route requests next route url" <|
        \_ ->
            let
                ...
                msgs =
                    Message.interpret msgIn (WithMaybeUrl maybeUrl)
            in
            case maybeUrl of
                Just url ->
    ...
    , test "KeyUp with play chord key plays a chord on MainRoute" <|
        \_ ->
            let
                ...
                msgs =
                    Message.interpret msgIn (WithRouteAndVolume MainRoute 0.5)
            in
    ...
    , test "KeyUp with a play note key plays a note on MainRoute" <|
        \_ ->
            let
                ...
                msgs =
                    Message.interpret msgIn (WithRouteVolumeNoteVIndexAndPitch MainRoute 0.5 4 42)
            in
    ...
    , test "KeyDown with next item on SelectChord route selects next chord" <|
        \_ ->
          let
              ...
              msgs =
                  Message.interpret msgIn (WithRouteAndSequenceItem SelectChordRoute A_Chord)
          in
