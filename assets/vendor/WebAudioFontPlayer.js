...
class WebAudioFontPlayer {
    ...
    playNote (audioContext, target, preset, pitch, duration, volume, slides) {
      this.queueWaveTable(audioContext, target, preset, 0, pitch, duration, volume, slides)
    }
    stop (audioContext, target) {
    ...
    queueWaveTable (audioContext, target, preset, when, pitch, duration, volume, slides) {
        ...
        if (zone) {
            ...
            var loop = !(zone.loopStart < 1 || zone.loopStart >= zone.loopEnd)
            if (!loop) {
            ...
            this.buffers.set(target, audioBufferSourceNode)
            audioBufferSourceNode.playbackRate.setValueAtTime(playbackRate, 0);
            if (slides) {
                if (slides.length > 0) {
                    audioBufferSourceNode.playbackRate.setValueAtTime(playbackRate, when);
                    for (var i = 0; i < slides.length; i++) {
                        var newPlaybackRate = 1.0 * Math.pow(2, (100.0 * slides[i].pitch - baseDetune) / 1200.0);
                        var newWhen = when + slides[i].when;
                        audioBufferSourceNode.playbackRate.linearRampToValueAtTime(newPlaybackRate, newWhen);
                    }
                }
            }
            audioBufferSourceNode.buffer = zone.buffer;
            if (loop) {
                audioBufferSourceNode.loop = true;
                audioBufferSourceNode.loopStart = zone.loopStart / zone.sampleRate + ((zone.delay) ? zone.delay : 0);
                audioBufferSourceNode.loopEnd = zone.loopEnd / zone.sampleRate + ((zone.delay) ? zone.delay : 0);
            }
            else {
                audioBufferSourceNode.loop = false;
            }
            audioBufferSourceNode.connect(envelope);
            audioBufferSourceNode.start(startWhen, zone.delay);
    ...
    findEnvelope (audioContext, target) {
        var envelope = this.envelopes[target];
      ...
      // }
        if (!(envelope)) {
            ...
            this.envelopes.set(target, envelope);
        }
        return envelope;
    };
